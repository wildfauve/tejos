...
def womens_singles_round_5(womens_singles):
    ...
    TEAM.draw(womens_singles).match('5.2').winner(women.Pegula).in_sets(2)  # (  4) Jessica Pegula  OR  (   ) Marketa Vondrousova
    TEAM.draw(womens_singles, '5.3').matchup(1, women.Jabeur, 2, women.Rybakina).select(2, 2)  # (  6) Ons Jabeur  OR  (  3) Elena Rybakina
...
def mens_singles_round_5(mens_singles):
    ...
    TEAM.draw(mens_singles, '5.2').matchup(1, men.Medvedev, 2, men.Eubanks).select(1, 5)  # (  3) Daniil Medvedev  OR  (   ) Christopher Eubanks
...
def mens_singles_round_4(mens_singles):
    TEAM.draw(mens_singles).match('4.1').winner(men.Berrettini).in_sets(3)  # (  1) Carlos Alcaraz  OR  (   ) Matteo Berrettini
