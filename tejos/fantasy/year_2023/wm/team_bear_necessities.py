...
def womens_singles_round_3(womens_singles):
    TEAM.draw(womens_singles).match('3.1').winner(women).in_sets()  # (  1) Swiatek  OR  ( 30) Martic
    TEAM.draw(womens_singles).match('3.2').winner(women).in_sets()  # ( 23) Linette  OR  ( 14) Bencic
    TEAM.draw(womens_singles).match('3.3').winner(women).in_sets()  # ( 11) Kasatkina  OR  ( 19) Azarenka
    TEAM.draw(womens_singles).match('3.4').winner(women).in_sets()  # ( WC) Svitolina  OR  (  Q) Kenin
    TEAM.draw(womens_singles).match('3.5').winner(women).in_sets()  # (  4) Pegula  OR  (   ) Cocciaretto
    TEAM.draw(womens_singles).match('3.6').winner(women).in_sets()  # (   ) Tsurenko  OR  (   ) Bogdan
    TEAM.draw(womens_singles).match('3.7').winner(women).in_sets()  # (   ) Vondrousova  OR  ( 20) Vekic
    TEAM.draw(womens_singles).match('3.8').winner(women).in_sets()  # ( 32) Bouzkova  OR  (  5) Garcia
    ...
    TEAM.draw(womens_singles).match('3.11').winner(women).in_sets()  # ( 13) Haddad_Maia  OR  (   ) Cirstea
    TEAM.draw(womens_singles).match('3.12').winner(women).in_sets()  # ( WC) Boulter  OR  (  3) Rybakina
    TEAM.draw(womens_singles).match('3.13').winner(women).in_sets()  # TBD  OR  TBD
    TEAM.draw(womens_singles).match('3.14').winner(women).in_sets()  # ( 22) Potapova  OR  (  Q) Andreeva_Mirra
...
def mens_singles_round_3(mens_singles):
    ...
    TEAM.draw(mens_singles).match('3.3').winner(men).in_sets()  # ( 10) Tiafoe  OR  ( 21) Dimitrov
    ...
    TEAM.draw(mens_singles).match('3.6').winner(men).in_sets()  # (   ) Lehecka  OR  ( 16) Paul
    ...
    TEAM.draw(mens_singles).match('3.9').winner(men).in_sets()  # (  8) Sinner  OR  (   ) Halys
    TEAM.draw(mens_singles).match('3.10').winner(men).in_sets()  # (   ) Galan  OR  (   ) Ymer_Mikael
    TEAM.draw(mens_singles).match('3.11').winner(men).in_sets()  # (   ) Pella  OR  (   ) Safiullin
    TEAM.draw(mens_singles).match('3.12').winner(men).in_sets()  # ( 26) Shapovalov  OR  ( WC) Broady
    TEAM.draw(mens_singles).match('3.13').winner(men).in_sets()  # (  7) Rublev  OR  ( WC) Goffin
    TEAM.draw(mens_singles).match('3.14').winner(men).in_sets()  # ( 23) Bublik  OR  (  Q) Marterer
    TEAM.draw(mens_singles).match('3.15').winner(men).in_sets()  # ( 14) Musetti  OR  ( 17) Hurkacz
    TEAM.draw(mens_singles).match('3.16').winner(men).in_sets()  # (   ) Wawrinka  OR  (  2) Djokovic
...
def womens_singles_round_2(womens_singles):
    ...
    TEAM.draw(womens_singles).match('2.2').winner(women.Parry).in_sets(2)  # (   ) Diane Parry  OR  ( 30) Petra Martic
    TEAM.draw(womens_singles).match('2.3').winner(women.Linette).in_sets(3)  # ( 23) Magda Linette  OR  (   ) Barbora Strycova
    TEAM.draw(womens_singles).match('2.4').winner(women.Collins).in_sets(3)  # (   ) Danielle Collins  OR  ( 14) Belinda Bencic
    TEAM.draw(womens_singles).match('2.5').winner(women.Burrage).in_sets(3)  # ( 11) Daria Kasatkina  OR  ( WC) Jodie Burrage
    TEAM.draw(womens_singles).match('2.6').winner(women.Podoroska).in_sets(3)  # (   ) Nadia Podoroska  OR  ( 19) Victoria Azarenka
    TEAM.draw(womens_singles).match('2.7').winner(women.Mertens).in_sets(3)  # ( 28) Elise Mertens  OR  ( WC) Svitolina
    TEAM.draw(womens_singles).match('2.8').winner(women.Kenin).in_sets(3)  # (   ) Xinyu Wang  OR  (  Q) Sofia Kenin
    TEAM.draw(womens_singles).match('2.9').winner(women.Bucsa).in_sets(3)  # (  4) Jessica Pegula  OR  (   ) Cristina Bucsa
    TEAM.draw(womens_singles).match('2.10').winner(women.Cocciaretto).in_sets(2)  # (   ) Elisabetta Cocciaretto  OR  (   ) Rebeka Masarova
    TEAM.draw(womens_singles).match('2.11').winner(women.Tsurenko).in_sets(2)  # (   ) Katerina Siniakova  OR  (   ) Lesia Tsurenko
    TEAM.draw(womens_singles).match('2.12').winner(women.Bogdan).in_sets(3)  # (   ) Alycia Parks  OR  (   ) Ana Bogdan
    TEAM.draw(womens_singles).match('2.13').winner(women.Vondrousova).in_sets(3)  # ( 12) Veronika Kudermetova  OR  (   ) Marketa Vondrousova
    TEAM.draw(womens_singles).match('2.14').winner(women.Vekic).in_sets(4)  # (   ) Sloane Stephens  OR  ( 20) Donna Vekic
    TEAM.draw(womens_singles).match('2.15').winner(women.Bouzkova).in_sets(3)  # ( 32) Marie Bouzkova  OR  (   ) Anett Kontaveit
    TEAM.draw(womens_singles).match('2.16').winner(women.Fernandez).in_sets(3)  # (   ) Leylah Fernandez  OR  (  5) Caroline Garcia
    ...
    TEAM.draw(womens_singles).match('2.19').winner(women.Korpatsch).in_sets(3)  # (  Q) Natalija Stevanovic  OR  ( LL) Tamara Korpatsch
    TEAM.draw(womens_singles).match('2.20').winner(women).in_sets()  # (   ) Sasnovich  OR  (  9) Kvitova
    TEAM.draw(womens_singles).match('2.21').winner(women.Haddad_Maia).in_sets(3)  # ( 13) Beatriz Haddad Maia  OR  (   ) Jaqueline Cristian
    TEAM.draw(womens_singles).match('2.22').winner(women.Cirstea).in_sets(3)  # (   ) Sorana Cirstea  OR  ( 17) Jelena Ostapenko
    TEAM.draw(womens_singles).match('2.23').winner(women.Tomova).in_sets(3)  # (   ) Viktoriya Tomova  OR  ( WC) Katie Boulter
    ...
    TEAM.draw(womens_singles).match('2.27').winner(women.Juvan).in_sets(3)  # ( 22) Anastasia Potapova  OR  (  Q) Kaja Juvan
    ...
    TEAM.draw(womens_singles).match('2.30').winner(women.Brengle).in_sets(3)  # (   ) Madison Brengle  OR  ( 21) Ekaterina Alexandrova
...
def mens_singles_round_2(mens_singles):
    ...
    TEAM.draw(mens_singles).match('2.17').winner(men.Schwartzman).in_sets(4)  # (  8) Jannik Sinner  OR  (   ) Diego Schwartzman
    TEAM.draw(mens_singles).match('2.18').winner(men.Vukic).in_sets(4)  # (   ) Aleksandar Vukic  OR  (   ) Quentin Halys
    TEAM.draw(mens_singles).match('2.19').winner(men.Galan).in_sets(5)  # (   ) Daniel Elahi Galan  OR  (  Q) Oscar Otte
    ...
    TEAM.draw(mens_singles).match('2.25').winner(men.Karatsev).in_sets(5)  # (  7) Andrey Rublev  OR  (   ) Aslan Karatsev
    TEAM.draw(mens_singles).match('2.26').winner(men.Goffin).in_sets(4)  # (  Q) Tomas Barrios Vera  OR  ( WC) David Goffin
    TEAM.draw(mens_singles).match('2.27').winner(men.Bublik).in_sets(5)  # ( 23) Alexander Bublik  OR  (   ) J.J. Wolf
    TEAM.draw(mens_singles).match('2.28').winner(men.Mmoh).in_sets(5)  # (  Q) Maximilian Marterer  OR  (   ) Michael Mmoh
    TEAM.draw(mens_singles).match('2.29').winner(men.Munar).in_sets(5)  # ( 14) Lorenzo Musetti  OR  (   ) Jaume Munar
    TEAM.draw(mens_singles).match('2.30').winner(men.Hurkacz).in_sets(3)  # ( WC) Jan Choinski  OR  ( 17) Hubert Hurkacz
    TEAM.draw(mens_singles).match('2.31').winner(men.Etcheverry).in_sets(3)  # ( 29) Tomas Martin Etcheverry  OR  (   ) Stan Wawrinka
