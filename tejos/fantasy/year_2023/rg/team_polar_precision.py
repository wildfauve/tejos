...
def mens_singles_round_5(mens_singles):
    ...
    TEAM.draw(mens_singles).match('5.2').winner(men.Djokovic).in_sets(4)  # (  3) Djokovic  OR  ( 11) Khachanov
...
def mens_singles_round_3(mens_singles):
    ...
    TEAM.draw(mens_singles).match('3.2').winner(men.Musetti).in_sets(5)  # ( 17) Musetti  OR  ( 14) Norrie
    ...
    TEAM.draw(mens_singles).match('3.6').winner(men.Hurkacz).in_sets(5)  # (   ) Varillas  OR  ( 13) Hurkacz
    TEAM.draw(mens_singles).match('3.7').winner(men.Khachanov).in_sets(3)  # ( 11) Khachanov  OR  (  W) Kokkinakis
    TEAM.draw(mens_singles).match('3.8').winner(men.Sonego).in_sets(5)  # (   ) Sonego  OR  (  7) Rublev
    ...
    TEAM.draw(mens_singles).match('3.10').winner(men.Cerundolo_Francisco).in_sets(
        5)  # ( 23) Cerundolo_Francisco  OR  (  9) Fritz
    TEAM.draw(mens_singles).match('3.11').winner(men.Jarry).in_sets(5)  # (   ) Jarry  OR  (   ) Giron
    TEAM.draw(mens_singles).match('3.12').winner(men.Zhang_Zhizhen).in_sets(5)  # (   ) Zhang_Zhizhen  OR  (  4) Ruud
    TEAM.draw(mens_singles).match('3.13').winner(men.Dimitrov).in_sets(4)  # (   ) Altmaier  OR  ( 28) Dimitrov
    TEAM.draw(mens_singles).match('3.14').winner(men.Tiafoe).in_sets(4)  # ( 22) Zverev  OR  ( 12) Tiafoe
    TEAM.draw(mens_singles).match('3.15').winner(men.Etcheverry).in_sets(3)  # ( 15) Coric  OR  (   ) Etcheverry
    TEAM.draw(mens_singles).match('3.16').winner(men.Seyboth_Wild).in_sets(3)  # ( 27) Nishioka  OR  (  Q) Seyboth_Wild
    return TEAM
...
def womens_singles_round_3(womens_singles):
    ...
    TEAM.draw(womens_singles).match('3.2').winner(women.Andreescu).in_sets(3)  # (   ) Andreescu  OR  (   ) Tsurenko
    TEAM.draw(womens_singles).match('3.3').winner(women.Schmiedlova).in_sets(3)  # (   ) Schmiedlova  OR  (  Q) Day
    TEAM.draw(womens_singles).match('3.4').winner(women.Gauff).in_sets(2)  # (  Q) Andreeva_Mirra  OR  (  6) Gauff
    ...
    TEAM.draw(womens_singles).match('3.12').winner(women.Mertens).in_sets(2)  # ( 28) Mertens  OR  (  3) Pegula
    TEAM.draw(womens_singles).match('3.13').winner(women.Blinkova).in_sets(3)  # (   ) Blinkova  OR  (   ) Svitolina
    TEAM.draw(womens_singles).match('3.14').winner(women.Kasatkina).in_sets(2)  # (   ) Stearns  OR  (  9) Kasatkina
...
def mens_singles_round_2(mens_singles):
    ...
    TEAM.draw(mens_singles).match('2.17').winner(men.Rune).in_sets(3)  # (  6) Rune  OR  (   ) Monfils
    ...
    TEAM.draw(mens_singles).match('2.26').winner(men.Ruusuvuori).in_sets(4)  # (   ) Ruusuvuori  OR  ( 28) Dimitrov
    ...
    TEAM.draw(mens_singles).match('2.32').winner(men.Pella).in_sets(4)  # (   ) Pella  OR  (  Q) Seyboth_Wild
    return TEAM
...
def womens_singles_round_2(womens_singles):
    ...
    TEAM.draw(womens_singles).match('2.2').winner(women.Wang_Xinyu).in_sets(2)  # (   ) Peterson  OR  (   ) Wang_Xinyu
    TEAM.draw(womens_singles).match('2.3').winner(women.Navarro).in_sets(2)  # (   ) Andreescu  OR  (  W) Navarro
    TEAM.draw(womens_singles).match('2.4').winner(women.Davis).in_sets(2)  # (   ) Davis  OR  (   ) Tsurenko
...
def mens_singles_round_1(mens_singles):
    ...
    TEAM.draw(mens_singles).match('1.6').winner(men.Shevchenko).in_sets(
        4)  # (   ) Alexander Shevchenko  OR  (   ) Oscar Otte
    ...
    TEAM.draw(mens_singles).match('1.9').winner(men.Auger_Aliassime).in_sets(
        5)  # ( 10) Felix Auger-Aliassime  OR  (   ) Fabio Fognini
    TEAM.draw(mens_singles).match('1.10').winner(men.Diaz_Acosta).in_sets(
        4)  # (   ) Jason Kubler  OR  (  L) Diaz Acosta
    ...
    TEAM.draw(mens_singles).match('1.19').winner(men.Van_Assche).in_sets(
        5)  # (   ) Luca Van Assche  OR  (   ) Marco Cecchinato
    TEAM.draw(mens_singles).match('1.20').winner(men.Fils).in_sets(
        4)  # (  W) Arthur Fils  OR  ( 29) Alejandro Davidovich Fokina
    ...
    TEAM.draw(mens_singles).match('1.35').winner(men.Mpetshi_Perricard).in_sets(
        5)  # (  W) G.Mpetshi Perricard  OR  (  Q) GA.Olivieri
    TEAM.draw(mens_singles).match('1.36').winner(men.Kecmanovic).in_sets(
        5)  # (  Q) Andrea Vavassori  OR  ( 31) Miomir Kecmanovic
    ...
    TEAM.draw(mens_singles).match('1.46').winner(men.Zhang_Zhizhen).in_sets(
        5)  # (   ) Zhizhen Zhang  OR  (   ) Dusan Lajovic
    ...
    TEAM.draw(mens_singles).match('1.56').winner(men.Tiafoe).in_sets(
        3)  # (   ) Filip Krajinovic  OR  ( 12) Frances Tiafoe
    ...
    TEAM.draw(mens_singles).match('1.61').winner(men.Nishioka).in_sets(
        4)  # ( 27) Yoshihito Nishioka  OR  (   ) J.J. Wolf
    TEAM.draw(mens_singles).match('1.62').winner(men.Thompson).in_sets(
        5)  # (   ) Max Purcell  OR  (   ) Jordan Thompson
...
def womens_singles_round_1(womens_singles):
    ...
    TEAM.draw(womens_singles).match('1.10').winner(women.Bolsova).in_sets(
        2)  # (  L) Aliona Bolsova  OR  (   ) Kristina Kucova
    ...
    TEAM.draw(womens_singles).match('1.14').winner(women.Andreeva_Mirra).in_sets(
        3)  # (  Q) Mirra Andreeva  OR  (   ) Alison Riske-Amritraj
    ...
    TEAM.draw(womens_singles).match('1.23').winner(women.Marino).in_sets(
        2)  # (   ) Rebecca Marino  OR  (   ) Diana Shnaider
    ...
    TEAM.draw(womens_singles).match('1.25').winner(women.Kvitova).in_sets(
        3)  # ( 10) Petra Kvitova  OR  (   ) Elisabetta Cocciaretto
    ...
    TEAM.draw(womens_singles).match('1.27').winner(women.Kontaveit).in_sets(
        2)  # (   ) Anett Kontaveit  OR  (   ) Bernarda Pera
    ...
    TEAM.draw(womens_singles).match('1.31').winner(women.Janicijevic).in_sets(
        2)  # (  W) Selena Janicijevic  OR  (   ) Dodin
    TEAM.draw(womens_singles).match('1.32').winner(women.Jabeur).in_sets(
        3)  # (   ) Lucia Bronzetti  OR  (  7) Ons Jabeur
    ...
    TEAM.draw(womens_singles).match('1.35').winner(women.Teichmann).in_sets(
        2)  # (   ) Sara Errani  OR  (   ) Jil Teichmann
    ...
    TEAM.draw(womens_singles).match('1.41').winner(women.Samsonova).in_sets(
        2)  # ( 15) Liudmila Samsonova  OR  (   ) Katie Volynets
    TEAM.draw(womens_singles).match(
    ...
    TEAM.draw(womens_singles).match('1.43').winner(women.Brengle).in_sets(
        2)  # (   ) Mayar Sherif  OR  (   ) Madison Brengle
    TEAM.draw(womens_singles).match('1.44').winner(women.Potapova).in_sets(
        2)  # (  Q) Taylor Townsend  OR  ( 24) Anastasia Potapova
    ...
    TEAM.draw(womens_singles).match('1.50').winner(women.Blinkova).in_sets(
        3)  # (   ) Anna Blinkova  OR  (   ) Ysaline Bonaventure
    ...
    TEAM.draw(womens_singles).match('1.55').winner(women.Vondrousova).in_sets(
        2)  # (   ) Marketa Vondrousova  OR  (   ) Alycia Parks
    ...
    TEAM.draw(womens_singles).match('1.63').winner(women.Udvardy).in_sets(
        2)  # (   ) Panna Udvardy  OR  (  Q) I.Shymanovich
