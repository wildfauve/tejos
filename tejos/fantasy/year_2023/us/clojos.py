...
def womens_singles_round_7(womens_singles):
    TEAM.draw(womens_singles).match('7.1').winner(women.Gauff).in_sets(2)  # (  6) Coco Gauff  OR  (  2) Aryna Sabalenka
# womens_singles_round_7:END


# mens_singles_round_7:START
def mens_singles_round_7(mens_singles):
    TEAM.draw(mens_singles, '7.1').matchup(1, men.Djokovic, 2, men.Medvedev).select(1, 3)  # (  2) Novak Djokovic  OR  (  3) Daniil Medvedev
# mens_singles_round_7:END


# womens_singles_round_6:START
def womens_singles_round_6(womens_singles):
    TEAM.draw(womens_singles).match('6.1').winner(women.Gauff).in_sets(3)  # (  6) Coco Gauff  OR  ( 10) Karolina Muchova
    TEAM.draw(womens_singles).match('6.2').winner(women.Sabalenka).in_sets(3)  # ( 17) Madison Keys  OR  (  2) Aryna Sabalenka
...
def womens_singles_round_4(womens_singles):
    ...
    TEAM.draw(womens_singles).match('4.5').winner(women.Vondrousova).in_sets(2)  # (   ) Peyton Stearns  OR  (  9) Marketa Vondrousova
    TEAM.draw(womens_singles).match('4.6').winner(women.Pegula).in_sets(3)  # ( 17) Madison Keys  OR  (  3) Jessica Pegula
    TEAM.draw(womens_singles).match('4.7').winner(women.Jabeur).in_sets(3)  # ( 23) Qinwen Zheng  OR  (  5) Ons Jabeur
...
def mens_singles_round_4(mens_singles):
    ...
    TEAM.draw(mens_singles).match('4.2').winner(men.Sinner).in_sets(4)  # (  6) Jannik Sinner  OR  ( 12) Alexander Zverev
    TEAM.draw(mens_singles).match('4.3').winner(men.Medvedev).in_sets(4)  # (  3) Daniil Medvedev  OR  ( 13) Alex de Minaur
...
def womens_singles_round_3(womens_singles):
    ...
    TEAM.draw(womens_singles, '3.2').matchup(1, women.Ostapenko, 2, women.Pera).select(1, 2)  # ( 20) Jelena Ostapenko  OR  (   ) Bernarda Pera
    TEAM.draw(womens_singles, '3.3').matchup(1, women.Caroline_Wozniacki, 2, women.Brady).select(2, 2)  # ( WC) Caroline Wozniacki  OR  (   ) Jennifer Brady
    TEAM.draw(womens_singles, '3.4').matchup(1, women.Mertens, 2, women.Gauff).select(2, 3)  # ( 32) Elise Mertens  OR  (  6) Coco Gauff
    TEAM.draw(womens_singles, '3.5').matchup(1, women.Cirstea, 2, women.Rybakina).select(2, 2)  # ( 30) Sorana Cirstea  OR  (  4) Elena Rybakina
    TEAM.draw(womens_singles, '3.6').matchup(1, women.Bencic, 2, women.Zhu).select(1, 2)  # ( 15) Belinda Bencic  OR  (   ) Lin Zhu
    TEAM.draw(womens_singles, '3.7').matchup(1, women.Muchova, 2, women.Townsend).select(1, 3)  # ( 10) Karolina Muchova  OR  (   ) Taylor Townsend
    TEAM.draw(womens_singles, '3.8').matchup(1, women.Wang_Xinyu, 2, women.Schmiedlova).select(1, 3)  # (   ) Xinyu Wang  OR  (   ) Anna Karolina Schmiedlova
    TEAM.draw(womens_singles, '3.9').matchup(1, women.Boulter, 2, women.Stearns).select(2, 3)  # (   ) Katie Boulter  OR  (   ) Peyton Stearns
    TEAM.draw(womens_singles, '3.10').matchup(1, women.Alexandrova, 2, women.Vondrousova).select(2, 2)  # ( 22) Ekaterina Alexandrova  OR  (  9) Marketa Vondrousova
    TEAM.draw(womens_singles, '3.11').matchup(1, women.Samsonova, 2, women.Keys).select(1, 2)  # ( 14) Liudmila Samsonova  OR  ( 17) Madison Keys
    TEAM.draw(womens_singles, '3.12').matchup(1, women.Svitolina, 2, women.Pegula).select(2, 3)  # ( 26) Svitolina  OR  (  3) Jessica Pegula
    TEAM.draw(womens_singles, '3.13').matchup(1, women.Jabeur, 2, women.Bouzkova).select(1, 2)  # (  5) Ons Jabeur  OR  ( 31) Marie Bouzkova
    TEAM.draw(womens_singles, '3.14').matchup(1, women.Zheng, 2, women.Bronzetti).select(1, 3)  # ( 23) Qinwen Zheng  OR  (   ) Lucia Bronzetti
...
def mens_singles_round_3(mens_singles):
    ...
    TEAM.draw(mens_singles, '3.2').matchup(1, men.Arnaldi, 2, men.Norrie).select(2, 4)  # (   ) Matteo Arnaldi  OR  ( 16) Cameron Norrie
    TEAM.draw(mens_singles, '3.3').matchup(1, men.Zverev, 2, men.Dimitrov).select(1, 5)  # ( 12) Alexander Zverev  OR  ( 19) Grigor Dimitrov
    TEAM.draw(mens_singles, '3.4').matchup(1, men.Sinner, 2, men.Wawrinka).select(1, 4)  # (  6) Jannik Sinner  OR  (   ) Stan Wawrinka
    TEAM.draw(mens_singles, '3.5').matchup(1, men.Medvedev, 2, men.Baez).select(1, 5)  # (  3) Daniil Medvedev  OR  (   ) Sebastian Baez
    TEAM.draw(mens_singles, '3.6').matchup(1, men.Alex_de_Minaur, 2, men.Jarry).select(2, 4)  # ( 13) Alex de Minaur  OR  ( 23) Nicolas Jarry
    TEAM.draw(mens_singles, '3.7').matchup(1, men.Mmoh, 2, men.Draper).select(2, 4)  # ( WC) Michael Mmoh  OR  (   ) Jack Draper
    TEAM.draw(mens_singles, '3.8').matchup(1, men.Rinderknech, 2, men.Rublev).select(2, 3)  # (   ) Arthur Rinderknech  OR  (  8) Andrey Rublev
    TEAM.draw(mens_singles, '3.9').matchup(1, men.Zhang_Zhizhen, 2, men.Hijikata).select(1, 4)  # (   ) Zhizhen Zhang  OR  ( WC) Rinky Hijikata
    TEAM.draw(mens_singles, '3.10').matchup(1, men.Mannarino, 2, men.Tiafoe).select(2, 4)  # ( 22) Adrian Mannarino  OR  ( 10) Frances Tiafoe
    TEAM.draw(mens_singles, '3.11').matchup(1, men.Paul, 2, men.Davidovich_Fokina).select(1, 4)  # ( 14) Tommy Paul  OR  ( 21) Alejandro Davidovich Fokina
    TEAM.draw(mens_singles, '3.12').matchup(1, men.Karatsev, 2, men.Shelton).select(2, 4)  # (   ) Aslan Karatsev  OR  (   ) Ben Shelton
    TEAM.draw(mens_singles, '3.13').matchup(1, men.Stricker, 2, men.Bonzi).select(1, 5)  # (  Q) D.Stricker  OR  ( WC) Benjamin Bonzi
    TEAM.draw(mens_singles, '3.14').matchup(1, men.Jakub_Mensik, 2, men.Fritz).select(2, 3)  # (  Q) Jakub Mensik  OR  (  9) Taylor Fritz
    TEAM.draw(mens_singles, '3.15').matchup(1, men.Gojo, 2, men.Vesely).select(2, 4)  # (  Q) Borna Gojo  OR  (   ) Jiri Vesely
...
def womens_singles_round_2(womens_singles):
    ...
    TEAM.draw(womens_singles).match('2.2').winner(women.Davis).in_sets(3)  # (   ) Lauren Davis  OR  (  Q) Kaja Juvan
    TEAM.draw(womens_singles).match('2.3').winner(women.Ostapenko).in_sets(2)  # ( 20) Jelena Ostapenko  OR  (   ) Elina Avanesyan
    TEAM.draw(womens_singles).match('2.4').winner(women.Wang_Xiyu).in_sets(2)  # (   ) Xiyu Wang  OR  (   ) Bernarda Pera
    TEAM.draw(womens_singles).match('2.5').winner(women.Kvitova).in_sets(2)  # ( 11) Petra Kvitova  OR  ( WC) Caroline Wozniacki
    TEAM.draw(womens_singles).match('2.6').winner(women.Linette).in_sets(2)  # (   ) Jennifer Brady  OR  ( 24) Magda Linette
    TEAM.draw(womens_singles).match('2.7').winner(women.Mertens).in_sets(3)  # ( 32) Elise Mertens  OR  (   ) Danielle Collins
    ...
    TEAM.draw(womens_singles).match('2.10').winner(women.Cirstea).in_sets(3)  # (   ) Anna Kalinskaya  OR  ( 30) Sorana Cirstea
    TEAM.draw(womens_singles).match('2.11').winner(women.Azarenka).in_sets(3)  # ( 18) Victoria Azarenka  OR  (   ) Lin Zhu
    TEAM.draw(womens_singles).match('2.12').winner(women.Bencic).in_sets(2)  # (  Q) Yuriko Lily Miyazaki  OR  ( 15) Belinda Bencic
    TEAM.draw(womens_singles).match('2.13').winner(women.Muchova).in_sets(2)  # ( 10) Karolina Muchova  OR  (   ) Magdalena Frech
    TEAM.draw(womens_singles).match('2.14').winner(women.Haddad_Maia).in_sets(3)  # (   ) Taylor Townsend  OR  ( 19) Beatriz Haddad Maia
    TEAM.draw(womens_singles).match('2.15').winner(women.Wang_Xinyu).in_sets(3)  # (   ) Sorribes Tormo  OR  (   ) Xinyu Wang
    TEAM.draw(womens_singles).match('2.16').winner(women.Schmiedlova).in_sets(3)  # (   ) Anna Karolina Schmiedlova  OR  (   ) Rebeka Masarova
    TEAM.draw(womens_singles).match('2.17').winner(women.Boulter).in_sets(2)  # (  Q) Yafan Wang  OR  (   ) Katie Boulter
    TEAM.draw(womens_singles).match('2.18').winner(women.Stearns).in_sets(2)  # (   ) Peyton Stearns  OR  (   ) Clara Tauson
    TEAM.draw(womens_singles).match('2.19').winner(women.Alexandrova).in_sets(3)  # ( 22) Ekaterina Alexandrova  OR  (   ) Lesia Tsurenko
    TEAM.draw(womens_singles).match('2.20').winner(women.Vondrousova).in_sets(2)  # (  9) Marketa Vondrousova  OR  (   ) Martina Trevisan
    TEAM.draw(womens_singles).match('2.21').winner(women.Samsonova).in_sets(2)  # ( 14) Liudmila Samsonova  OR  (   ) Tamara Korpatsch
    TEAM.draw(womens_singles).match('2.22').winner(women.Keys).in_sets(2)  # ( LL) Yanina Wickmayer  OR  ( 17) Madison Keys
    TEAM.draw(womens_singles).match('2.23').winner(women.Svitolina).in_sets(3)  # ( 26) Svitolina  OR  (   ) Anastasia Pavlyuchenkova
    ...
    TEAM.draw(womens_singles).match('2.26').winner(women.Bouzkova).in_sets(3)  # (   ) Petra Martic  OR  ( 31) Marie Bouzkova
    TEAM.draw(womens_singles).match('2.27').winner(women.Zheng).in_sets(3)  # ( 23) Qinwen Zheng  OR  (   ) Kaia Kanepi
    TEAM.draw(womens_singles).match('2.28').winner(women.Bronzetti).in_sets(3)  # (  Q) Eva Lys  OR  (   ) Lucia Bronzetti
    TEAM.draw(womens_singles, '2.29').matchup(1, women.Kasatkina, 2, women.Kenin).select(1, 2)  # ( 13) Daria Kasatkina  OR  (   ) Sofia Kenin
    TEAM.draw(womens_singles).match('2.30').winner(women.Sachia_Vickery).in_sets(3)  # (  Q) Greet Minnen  OR  (  Q) Sachia Vickery
    TEAM.draw(womens_singles).match('2.31').winner(women.Pliskova).in_sets(2)  # ( 25) Karolina Pliskova  OR  (   ) Clara Burel
...
def mens_singles_round_2(mens_singles):
    ...
    TEAM.draw(mens_singles, '2.3').matchup(1, men.Fils, 2, men.Arnaldi).select(1, 4)  # (   ) Arthur Fils  OR  (   ) Matteo Arnaldi
    TEAM.draw(mens_singles).match('2.4').winner(men.Norrie).in_sets(3)  # ( 16) Cameron Norrie  OR  (  Q) Yu Hsiou Hsu
    TEAM.draw(mens_singles).match('2.5').winner(men.Zverev).in_sets(3)  # ( 12) Alexander Zverev  OR  (   ) Daniel Altmaier
    TEAM.draw(mens_singles).match('2.6').winner(men.Murray_Andy).in_sets(4)  # (   ) Andy Murray  OR  ( 19) Grigor Dimitrov
    TEAM.draw(mens_singles, '2.7').matchup(1, men.Wawrinka, 2, men.Etcheverry).select(2, 4)  # (   ) Stan Wawrinka  OR  ( 30) Tomas Martin Etcheverry
    TEAM.draw(mens_singles).match('2.8').winner(men.Sinner).in_sets(3)  # (   ) Lorenzo Sonego  OR  (  6) Jannik Sinner
    TEAM.draw(mens_singles).match('2.9').winner(men.Medvedev).in_sets(4)  # (  3) Daniil Medvedev  OR  (   ) Christopher O'Connell
    TEAM.draw(mens_singles).match('2.10').winner(men.Baez).in_sets(4)  # (  Q) Felipe Meligeni Alves  OR  (   ) Sebastian Baez
    TEAM.draw(mens_singles).match('2.11').winner(men.Jarry).in_sets(4)  # ( 23) Nicolas Jarry  OR  ( WC) Alex Michelsen
    TEAM.draw(mens_singles).match('2.12').winner(men.Alex_de_Minaur).in_sets(4)  # ( 13) Alex de Minaur  OR  (   ) Yibing Wu
    TEAM.draw(mens_singles).match('2.13').winner(men.Mmoh).in_sets(5)  # ( WC) Michael Mmoh  OR  ( WC) John Isner
    TEAM.draw(mens_singles).match('2.14').winner(men.Hurkacz).in_sets(4)  # (   ) Jack Draper  OR  ( 17) Hubert Hurkacz
    TEAM.draw(mens_singles).match('2.15').winner(men.Berrettini).in_sets(4)  # (   ) Matteo Berrettini  OR  (   ) Arthur Rinderknech
    TEAM.draw(mens_singles).match('2.16').winner(men.Rublev).in_sets(4)  # (  8) Andrey Rublev  OR  (   ) Gael Monfils
    TEAM.draw(mens_singles).match('2.17').winner(men.Ruud).in_sets(3)  # (  5) Casper Ruud  OR  (   ) Zhizhen Zhang
    TEAM.draw(mens_singles).match('2.18').winner(men.Fucsovics).in_sets(4)  # ( WC) Rinky Hijikata  OR  (   ) Marton Fucsovics
    TEAM.draw(mens_singles).match('2.19').winner(men.Mannarino).in_sets(4)  # ( 22) Adrian Mannarino  OR  (   ) Fabian Marozsan
    TEAM.draw(mens_singles).match('2.20').winner(men.Tiafoe).in_sets(4)  # (   ) Sebastian Ofner  OR  ( 10) Frances Tiafoe
    TEAM.draw(mens_singles).match('2.21').winner(men.Paul).in_sets(4)  # ( 14) Tommy Paul  OR  (   ) Roman Safiullin
    TEAM.draw(mens_singles).match('2.22').winner(men.Davidovich_Fokina).in_sets(5)  # (   ) Juan Manuel Cerundolo  OR  ( 21) Alejandro Davidovich Fokina
    TEAM.draw(mens_singles).match('2.23').winner(men.Shelton).in_sets(4)  # (   ) Dominic Thiem  OR  (   ) Ben Shelton
    TEAM.draw(mens_singles).match('2.24').winner(men.Karatsev).in_sets(4)  # (   ) Aslan Karatsev  OR  (   ) Roberto Carballes Baena
    TEAM.draw(mens_singles).match('2.25').winner(men.Tsitsipas).in_sets(5)  # (  7) Stefanos Tsitsipas  OR  (  Q) D.Stricker
    TEAM.draw(mens_singles).match('2.26').winner(men.Eubanks).in_sets(4)  # ( WC) Benjamin Bonzi  OR  ( 28) Christopher Eubanks
    TEAM.draw(mens_singles).match('2.27').winner(men.Jakub_Mensik).in_sets(4)  # (  Q) Titouan Droguet  OR  (  Q) Jakub Mensik
    TEAM.draw(mens_singles).match('2.28').winner(men.Fritz).in_sets(4)  # (   ) Juan Pablo Varillas  OR  (  9) Taylor Fritz
    TEAM.draw(mens_singles).match('2.29').winner(men.Mcdonald).in_sets(4)  # (   ) Mackenzie McDonald  OR  (  Q) Borna Gojo
    TEAM.draw(mens_singles).match('2.30').winner(men.Cerundolo_Francisco).in_sets(4)  # (   ) Jiri Vesely  OR  ( 20) Francisco Cerundolo
    TEAM.draw(mens_singles).match('2.31').winner(men.Djere).in_sets(4)  # ( 32) Laslo Djere  OR  (  Q) Hugo Gaston
...
def womens_singles_round_1(womens_singles):
    ...
    TEAM.draw(womens_singles).match('1.2').winner(women.Saville).in_sets(2)  # ( WC) Clervie Ngounoue  OR  (   ) Daria Saville
    TEAM.draw(womens_singles).match('1.3').winner(women.Kovinic).in_sets(3)  # (   ) Lauren Davis  OR  (   ) Danka Kovinic
    TEAM.draw(womens_singles).match('1.4').winner(women.Cocciaretto).in_sets(3)  # (  Q) Kaja Juvan  OR  ( 29) Elisabetta Cocciaretto
    TEAM.draw(womens_singles).match('1.5').winner(women.Ostapenko).in_sets(2)  # ( 20) Jelena Ostapenko  OR  (   ) Jasmine Paolini
    TEAM.draw(womens_singles).match('1.6').winner(women.Avanesyan).in_sets(3)  # (   ) Alizé Cornet  OR  (   ) Elina Avanesyan
    TEAM.draw(womens_singles).match('1.7').winner(women.Hruncakova).in_sets(3)  # (   ) V.Hruncakova  OR  (   ) Xiyu Wang
    ...
    TEAM.draw(womens_singles).match('1.10').winner(women.Tatiana_Prozorova).in_sets(3)  # (  Q) Tatiana Prozorova  OR  ( WC) Caroline Wozniacki
    TEAM.draw(womens_singles).match('1.11').winner(women.Brady).in_sets(2)  # ( LL) Kimberly Birrell  OR  (   ) Jennifer Brady
    TEAM.draw(womens_singles).match('1.12').winner(women.Linette).in_sets(2)  # (   ) Aliaksandra Sasnovich  OR  ( 24) Magda Linette
    TEAM.draw(womens_singles).match('1.13').winner(women.Mertens).in_sets(2)  # ( 32) Elise Mertens  OR  (  Q) Mirjam Bjorklund
    TEAM.draw(womens_singles).match('1.14').winner(women.Fruhvirtova_Linda).in_sets(3)  # (   ) Danielle Collins  OR  (   ) Linda Fruhvirtova
    TEAM.draw(womens_singles).match('1.15').winner(women.Andreeva_Mirra).in_sets(2)  # (  Q) Olivia Gadecki  OR  (   ) Mirra Andreeva
    ...
    TEAM.draw(womens_singles).match('1.18').winner(women.Udvardy).in_sets(3)  # (   ) Panna Udvardy  OR  (   ) Ajla Tomljanovic
    TEAM.draw(womens_singles).match('1.19').winner(women.Kalinskaya).in_sets(2)  # (   ) Anna Kalinskaya  OR  (   ) Katerina Siniakova
    ...
    TEAM.draw(womens_singles).match('1.22').winner(women.Zhu).in_sets(3)  # (   ) Lin Zhu  OR  (   ) Mayar Sherif
    TEAM.draw(womens_singles).match('1.23').winner(women.Betova).in_sets(2)  # (  Q) Yuriko Lily Miyazaki  OR  (   ) Margarita Betova
    TEAM.draw(womens_singles).match('1.24').winner(women.Bencic).in_sets(2)  # (   ) Kamilla Rakhimova  OR  ( 15) Belinda Bencic
    TEAM.draw(womens_singles).match('1.25').winner(women.Muchova).in_sets(2)  # ( 10) Karolina Muchova  OR  ( WC) Storm Hunter
    TEAM.draw(womens_singles).match('1.26').winner(women.Frech).in_sets(3)  # (   ) Magdalena Frech  OR  (   ) Emma Navarro
    TEAM.draw(womens_singles).match('1.27').winner(women.Gracheva).in_sets(2)  # (   ) Varvara Gracheva  OR  (   ) Taylor Townsend
    TEAM.draw(womens_singles).match('1.28').winner(women.Haddad_Maia).in_sets(2)  # (   ) Sloane Stephens  OR  ( 19) Beatriz Haddad Maia
    TEAM.draw(womens_singles).match('1.29').winner(women.Kalinina).in_sets(3)  # ( 28) Anhelina Kalinina  OR  (   ) Sorribes Tormo
    TEAM.draw(womens_singles).match('1.30').winner(women.Wang_Xinyu).in_sets(2)  # (  Q) Katie Volynets  OR  (   ) Xinyu Wang
    TEAM.draw(womens_singles).match('1.31').winner(women.Schmiedlova).in_sets(2)  # (   ) Anna Karolina Schmiedlova  OR  (   ) Kateryna Baindl
    TEAM.draw(womens_singles).match('1.32').winner(women.Masarova).in_sets(2)  # (   ) Rebeka Masarova  OR  (  8) Maria Sakkari
...
def mens_singles_round_1(mens_singles):
    ...
    TEAM.draw(mens_singles).match('1.2').winner(men.Pella).in_sets(4)  # (   ) Lloyd Harris  OR  (   ) Guido Pella
    TEAM.draw(mens_singles).match('1.3').winner(men.Thompson).in_sets(4)  # (   ) Botic van De Zandschulp  OR  (   ) Jordan Thompson
    TEAM.draw(mens_singles).match('1.4').winner(men.Galan).in_sets(4)  # (   ) Daniel Elahi Galan  OR  ( 26) Daniel Evans
    TEAM.draw(mens_singles).match('1.5').winner(men.Griekspoor).in_sets(3)  # ( 24) Tallon Griekspoor  OR  (   ) Arthur Fils
    TEAM.draw(mens_singles).match('1.6').winner(men.Kubler).in_sets(4)  # (   ) Jason Kubler  OR  (   ) Matteo Arnaldi
    TEAM.draw(mens_singles).match('1.7').winner(men.Kokkinakis).in_sets(3)  # (   ) Thanasi Kokkinakis  OR  (  Q) Yu Hsiou Hsu
    TEAM.draw(mens_singles).match('1.8').winner(men.Norrie).in_sets(3)  # (   ) Alexander Shevchenko  OR  ( 16) Cameron Norrie
    TEAM.draw(mens_singles).match('1.9').winner(men.Zverev).in_sets(3)  # ( 12) Alexander Zverev  OR  (   ) Aleksandar Vukic
    TEAM.draw(mens_singles).match('1.10').winner(men.Altmaier).in_sets(4)  # (   ) Daniel Altmaier  OR  (   ) Constant Lestienne
    TEAM.draw(mens_singles).match('1.11').winner(men.Murray_Andy).in_sets(4)  # (   ) Andy Murray  OR  (   ) Corentin Moutet
    TEAM.draw(mens_singles).match('1.12').winner(men.Dimitrov).in_sets(3)  # (   ) Alex Molcan  OR  ( 19) Grigor Dimitrov
    TEAM.draw(mens_singles).match('1.13').winner(men.Etcheverry).in_sets(4)  # ( 30) Tomas Martin Etcheverry  OR  (  Q) Otto Virtanen
    TEAM.draw(mens_singles).match('1.14').winner(men.Wawrinka).in_sets(4)  # (   ) Stan Wawrinka  OR  (   ) Yoshihito Nishioka
    TEAM.draw(mens_singles).match('1.15').winner(men.Sonego).in_sets(3)  # (  Q) Nicolas Moreno De Alboran  OR  (   ) Lorenzo Sonego
    ...
    TEAM.draw(mens_singles).match('1.18').winner(men.OConnell).in_sets(4)  # (   ) Max Purcell  OR  (   ) Christopher O'Connell
    TEAM.draw(mens_singles).match('1.19').winner(men.Duckworth).in_sets(4)  # (   ) James Duckworth  OR  (  Q) Felipe Meligeni Alves
    TEAM.draw(mens_singles).match('1.20').winner(men.Coric).in_sets(4)  # (   ) Sebastian Baez  OR  ( 27) Borna Coric
    TEAM.draw(mens_singles).match('1.21').winner(men.Jarry).in_sets(3)  # ( 23) Nicolas Jarry  OR  (   ) Luca Van Assche
    TEAM.draw(mens_singles).match('1.22').winner(men.Ramos_Vinolas).in_sets(4)  # (   ) Albert Ramos-Vinolas  OR  ( WC) Alex Michelsen
    TEAM.draw(mens_singles).match('1.23').winner(men.Wu).in_sets(4)  # (   ) Yibing Wu  OR  (   ) Dusan Lajovic
    TEAM.draw(mens_singles).match('1.24').winner(men.Alex_de_Minaur).in_sets(3)  # (  Q) Timofey Skatov  OR  ( 13) Alex de Minaur
    TEAM.draw(mens_singles).match('1.25').winner(men.Khachanov).in_sets(3)  # ( 11) Karen Khachanov  OR  ( WC) Michael Mmoh
    TEAM.draw(mens_singles).match('1.26').winner(men.Facundo_Diaz_Acosta).in_sets(4)  # (   ) Facundo Diaz Acosta  OR  ( WC) John Isner
    TEAM.draw(mens_singles).match('1.27').winner(men.Albot).in_sets(4)  # (   ) Radu Albot  OR  (   ) Jack Draper
    TEAM.draw(mens_singles).match('1.28').winner(men.Hurkacz).in_sets(3)  # (   ) Marc-Andrea Huesler  OR  ( 17) Hubert Hurkacz
    TEAM.draw(mens_singles).match('1.29').winner(men.Humbert).in_sets(4)  # ( 29) Ugo Humbert  OR  (   ) Matteo Berrettini
    TEAM.draw(mens_singles).match('1.30').winner(men.Rinderknech).in_sets(5)  # (   ) Diego Schwartzman  OR  (   ) Arthur Rinderknech
    TEAM.draw(mens_singles).match('1.31').winner(men.Monfils).in_sets(4)  # (  Q) Taro Daniel  OR  (   ) Gael Monfils
    TEAM.draw(mens_singles).match('1.32').winner(men.Rublev).in_sets(3)  # (   ) Arthur Cazaux  OR  (  8) Andrey Rublev
