...
def womens_singles_round_2(womens_singles):
    ...
    TEAM.draw(womens_singles, '2.2').matchup(1, women.Davis, 2, women.Juvan).select(1, 3)  # (   ) Lauren Davis  OR  (  Q) Kaja Juvan
    TEAM.draw(womens_singles, '2.3').matchup(1, women.Ostapenko, 2, women.Avanesyan).select(1, 2)  # ( 20) Jelena Ostapenko  OR  (   ) Elina Avanesyan
    TEAM.draw(womens_singles, '2.4').matchup(1, women.Wang_Xiyu, 2, women.Pera).select(1, 2)  # (   ) Xiyu Wang  OR  (   ) Bernarda Pera
    TEAM.draw(womens_singles, '2.5').matchup(1, women.Kvitova, 2, women.Caroline_Wozniacki).select(1, 2)  # ( 11) Petra Kvitova  OR  ( WC) Caroline Wozniacki
    TEAM.draw(womens_singles, '2.6').matchup(1, women.Brady, 2, women.Linette).select(2, 2)  # (   ) Jennifer Brady  OR  ( 24) Magda Linette
    TEAM.draw(womens_singles, '2.7').matchup(1, women.Mertens, 2, women.Collins).select(1, 3)  # ( 32) Elise Mertens  OR  (   ) Danielle Collins
    ...
    TEAM.draw(womens_singles, '2.10').matchup(1, women.Kalinskaya, 2, women.Cirstea).select(2, 3)  # (   ) Anna Kalinskaya  OR  ( 30) Sorana Cirstea
    TEAM.draw(womens_singles, '2.11').matchup(1, women.Azarenka, 2, women.Zhu).select(1, 3)  # ( 18) Victoria Azarenka  OR  (   ) Lin Zhu
    TEAM.draw(womens_singles, '2.12').matchup(1, women.Yuriko_Lily_Miyazaki, 2, women.Bencic).select(2, 2)  # (  Q) Yuriko Lily Miyazaki  OR  ( 15) Belinda Bencic
    TEAM.draw(womens_singles, '2.13').matchup(1, women.Muchova, 2, women.Frech).select(1, 2)  # ( 10) Karolina Muchova  OR  (   ) Magdalena Frech
    TEAM.draw(womens_singles, '2.14').matchup(1, women.Townsend, 2, women.Haddad_Maia).select(2, 3)  # (   ) Taylor Townsend  OR  ( 19) Beatriz Haddad Maia
    TEAM.draw(womens_singles, '2.15').matchup(1, women.Sorribes_Tormo, 2, women.Wang_Xinyu).select(2, 3)  # (   ) Sorribes Tormo  OR  (   ) Xinyu Wang
    TEAM.draw(womens_singles, '2.16').matchup(1, women.Schmiedlova, 2, women.Masarova).select(1, 3)  # (   ) Anna Karolina Schmiedlova  OR  (   ) Rebeka Masarova
...
def mens_singles_round_2(mens_singles):
    ...
    TEAM.draw(mens_singles, '2.4').matchup(1, men.Norrie, 2, men.Hsu).select(1, 3)  # ( 16) Cameron Norrie  OR  (  Q) Yu Hsiou Hsu
    TEAM.draw(mens_singles, '2.5').matchup(1, men.Zverev, 2, men.Altmaier).select(1, 3)  # ( 12) Alexander Zverev  OR  (   ) Daniel Altmaier
    TEAM.draw(mens_singles, '2.6').matchup(1, men.Murray_Andy, 2, men.Dimitrov).select(1, 4)  # (   ) Andy Murray  OR  ( 19) Grigor Dimitrov
    ...
    TEAM.draw(mens_singles, '2.9').matchup(1, men.Medvedev, 2, men.OConnell).select(1, 4)  # (  3) Daniil Medvedev  OR  (   ) Christopher O'Connell
    TEAM.draw(mens_singles, '2.10').matchup(1, men.Felipe_Meligeni_Alves, 2, men.Baez).select(2, 4)  # (  Q) Felipe Meligeni Alves  OR  (   ) Sebastian Baez
    TEAM.draw(mens_singles, '2.11').matchup(1, men.Jarry, 2, men.Alex_Michelsen).select(1, 4)  # ( 23) Nicolas Jarry  OR  ( WC) Alex Michelsen
    TEAM.draw(mens_singles, '2.12').matchup(1, men.Alex_de_Minaur, 2, men.Wu).select(1, 4)  # ( 13) Alex de Minaur  OR  (   ) Yibing Wu
    TEAM.draw(mens_singles, '2.13').matchup(1, men.Mmoh, 2, men.Isner).select(1, 5)  # ( WC) Michael Mmoh  OR  ( WC) John Isner
    TEAM.draw(mens_singles, '2.14').matchup(1, men.Draper, 2, men.Hurkacz).select(2, 4)  # (   ) Jack Draper  OR  ( 17) Hubert Hurkacz
    TEAM.draw(mens_singles, '2.15').matchup(1, men.Berrettini, 2, men.Rinderknech).select(1, 4)  # (   ) Matteo Berrettini  OR  (   ) Arthur Rinderknech
    TEAM.draw(mens_singles, '2.16').matchup(1, men.Rublev, 2, men.Monfils).select(1, 4)  # (  8) Andrey Rublev  OR  (   ) Gael Monfils
