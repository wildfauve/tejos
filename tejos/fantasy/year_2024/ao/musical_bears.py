...
def womens_singles_round_7(womens_singles):
    TEAM.draw(womens_singles, '7.1').matchup(1, women.Sabalenka, 2, women.Zheng).select(2,3)  # (  2) Aryna Sabalenka  OR  ( 12) Qinwen Zheng
# womens_singles_round_7:END


# mens_singles_round_7:START
def mens_singles_round_7(mens_singles):
    TEAM.draw(mens_singles, '7.1').matchup(1, men.Medvedev, 2, men.Sinner).select(1,4)  # (  3) Daniil Medvedev  OR  (  4) Jannik Sinner
...
def womens_singles_round_5(womens_singles):
    TEAM.draw(womens_singles, '5.1').matchup(1, women.Noskova, 2, women.Yastremska).select(1,2)  # (   ) Linda Noskova  OR  (  Q) Dayana Yastremska
    TEAM.draw(womens_singles, '5.2').matchup(1, women.Zheng, 2, women.Kalinskaya).select(2,3)  # ( 12) Qinwen Zheng  OR  (   ) Anna Kalinskaya
...
def womens_singles_round_4(womens_singles):
    ...
    TEAM.draw(womens_singles, '4.3').matchup(1, women.Paolini, 2, women.Kalinskaya).select(1,2)  # ( 26) Jasmine Paolini  OR  (   ) Anna Kalinskaya
    TEAM.draw(womens_singles, '4.4').matchup(1, women.Zheng, 2, women.Oceane_Dodin).select(1,2)  # ( 12) Qinwen Zheng  OR  (   ) Oceane Dodin
    TEAM.draw(womens_singles).match('4.5').winner(women.Kostyuk).in_sets(3)  # (   ) Marta Kostyuk  OR  (  Q) Maria Timofeeva
    TEAM.draw(womens_singles).match('4.6').winner(women.Gauff).in_sets(2)  # (   ) Magdalena Frech  OR  (  4) Coco Gauff
    TEAM.draw(womens_singles).match('4.7').winner(women.Krejcikova).in_sets(3)  # (   ) Mirra Andreeva  OR  (  9) Barbora Krejcikova
    TEAM.draw(womens_singles).match('4.8').winner(women.Sabalenka).in_sets(2)  # (  2) Aryna Sabalenka  OR  (   ) Amanda Anisimova
...
def mens_singles_round_4(mens_singles):
    ...
    TEAM.draw(mens_singles).match('4.2').winner(men.Tsitsipas).in_sets(5)  # ( 12) Taylor Fritz  OR  (  7) Stefanos Tsitsipas
    TEAM.draw(mens_singles, '4.3').matchup(1, men.Sinner, 2, men.Khachanov).select()  # (  4) Jannik Sinner  OR  ( 15) Karen Khachanov
    TEAM.draw(mens_singles).match('4.4').winner(men.Rublev).in_sets(5)  # (  5) Andrey Rublev  OR  ( 10) Alex de Minaur
    TEAM.draw(mens_singles, '4.5').matchup(1, men.Cazaux, 2, men.Hurkacz).select(2,5)  # ( WC) Arthur Cazaux  OR  (  9) Hubert Hurkacz
    TEAM.draw(mens_singles, '4.6').matchup(1, men.Medvedev, 2, men.Borges).select(1,4)  # (  3) Daniil Medvedev  OR  (   ) Nuno Borges
    TEAM.draw(mens_singles, '4.7').matchup(1, men.Norrie, 2, men.Zverev).select(1,5)  # ( 19) Cameron Norrie  OR  (  6) Alexander Zverev
...
def womens_singles_round_3(womens_singles):
    ...
    TEAM.draw(womens_singles, '3.5').matchup(1, women.Blinkova, 2, women.Paolini).select(1,3)  # (   ) Anna Blinkova  OR  ( 26) Jasmine Paolini
    TEAM.draw(womens_singles, '3.6').matchup(1, women.Kalinskaya, 2, women.Stephens).select(2,2)  # (   ) Anna Kalinskaya  OR  (   ) Sloane Stephens
    TEAM.draw(womens_singles, '3.7').matchup(1, women.Zheng, 2, women.Yafan_Wang).select(1,2)  # ( 12) Qinwen Zheng  OR  (   ) Yafan Wang
    ...
    TEAM.draw(womens_singles).match('3.10').winner(women.Haddad_Maia).in_sets(3)  # ( 10) Beatriz Haddad Maia  OR  (  Q) Maria Timofeeva
    TEAM.draw(womens_singles).match('3.11').winner(women.Frech).in_sets(2)  # (   ) Magdalena Frech  OR  (  Q) Anastasia Zakharova
    TEAM.draw(womens_singles).match('3.12').winner(women.Gauff).in_sets(3)  # (   ) Alycia Parks  OR  (  4) Coco Gauff
    TEAM.draw(womens_singles).match('3.13').winner(women.Andreeva_Mirra).in_sets(3)  # (   ) Mirra Andreeva  OR  (   ) Diane Parry
    ...
    TEAM.draw(womens_singles).match('3.16').winner(women.Sabalenka).in_sets(2)  # (  2) Aryna Sabalenka  OR  ( 28) Lesia Tsurenko
# womens_singles_round_3:END


# mens_singles_round_3:START
def mens_singles_round_3(mens_singles):
    TEAM.draw(mens_singles).match('3.1').winner(men.Djokovic).in_sets(3)  # ( 30) Tomas Martin Etcheverry  OR  (  1) Novak Djokovic
    TEAM.draw(mens_singles).match('3.2').winner(men.Shelton).in_sets(3)  # ( 20) Adrian Mannarino  OR  ( 16) Ben Shelton
    TEAM.draw(mens_singles).match('3.3').winner(men.Fritz).in_sets(3)  # ( 12) Taylor Fritz  OR  (   ) Fabian Marozsan
    TEAM.draw(mens_singles).match('3.4').winner(men.Van_Assche).in_sets(4)  # (   ) Luca Van Assche  OR  (  7) Stefanos Tsitsipas
    TEAM.draw(mens_singles).match('3.5').winner(men.Sinner).in_sets(4)  # (  4) Jannik Sinner  OR  ( 26) Sebastian Baez
    TEAM.draw(mens_singles).match('3.6').winner(men.Khachanov).in_sets(5)  # ( 15) Karen Khachanov  OR  (   ) Tomas Machac
    TEAM.draw(mens_singles).match('3.7').winner(men.Alex_de_Minaur).in_sets(5)  # (  Q) Flavio Cobolli  OR  ( 10) Alex de Minaur
    ...
    TEAM.draw(mens_singles, '3.12').matchup(1, men.Medvedev, 2, men.Auger_Aliassime).select(1,4)  # (  3) Daniil Medvedev  OR  ( 27) Felix Auger-Aliassime
...
def womens_singles_round_2(womens_singles):
    ...
    TEAM.draw(womens_singles).match('2.2').winner(women.Noskova).in_sets(2)  # ( WC) McCartney Kessler  OR  (   ) Linda Noskova
    ...
    TEAM.draw(womens_singles, '2.6').matchup(1, women.Azarenka, 2, women.Clara_Tauson).select(1,3)  # ( 18) Victoria Azarenka  OR  (   ) Clara Tauson
    ...
    TEAM.draw(womens_singles).match('2.8').winner(women.Gracheva).in_sets(3)  # (   ) Varvara Gracheva  OR  (  Q) Dayana Yastremska
    TEAM.draw(womens_singles, '2.9').matchup(1, women.Blinkova, 2, women.Rybakina).select(1,2)  # (   ) Anna Blinkova  OR  (  3) Elena Rybakina
    TEAM.draw(womens_singles).match('2.10').winner(women.Paolini).in_sets(3)  # (   ) Tatjana Maria  OR  ( 26) Jasmine Paolini
    TEAM.draw(womens_singles).match('2.11').winner(women.Rus).in_sets(3)  # (   ) Arantxa Rus  OR  (   ) Anna Kalinskaya
    TEAM.draw(womens_singles, '2.12').matchup(1, women.Kasatkina, 2, women.Stephens).select(2,3)  # ( 14) Daria Kasatkina  OR  (   ) Sloane Stephens
    TEAM.draw(womens_singles, '2.13').matchup(1, women.Boulter, 2, women.Zheng).select(2,3)  # (   ) Katie Boulter  OR  ( 12) Qinwen Zheng
    ...
    TEAM.draw(womens_singles, '2.16').matchup(1, women.Burel, 2, women.Pegula).select(2,3)  # (   ) Clara Burel  OR  (  5) Jessica Pegula
    ...
    TEAM.draw(womens_singles).match('2.20').winner(women.Haddad_Maia).in_sets(2)  # ( 10) Beatriz Haddad Maia  OR  (  Q) Alina Korneeva
    TEAM.draw(womens_singles).match('2.21').winner(women.Garcia).in_sets(2)  # ( 16) Caroline Garcia  OR  (   ) Magdalena Frech
    TEAM.draw(womens_singles).match('2.22').winner(women.Juvan).in_sets(3)  # (  Q) Anastasia Zakharova  OR  (   ) Kaja Juvan
    TEAM.draw(womens_singles).match('2.23').winner(women.Fernandez).in_sets(3)  # (   ) Alycia Parks  OR  ( 32) Leylah Fernandez
    ...
    TEAM.draw(womens_singles).match('2.26').winner(women.Rakhimova).in_sets(3)  # (   ) Kamilla Rakhimova  OR  (   ) Diane Parry
    TEAM.draw(womens_singles).match('2.27').winner(women.Siegemund).in_sets(3)  # (  Q) Storm Hunter  OR  (   ) Laura Siegemund
    TEAM.draw(womens_singles).match('2.28').winner(women.Krejcikova).in_sets(2)  # (   ) Tamara Korpatsch  OR  (  9) Barbora Krejcikova
    TEAM.draw(womens_singles).match('2.29').winner(women.Podoroska).in_sets(2)  # (   ) Nadia Podoroska  OR  (   ) Amanda Anisimova
    TEAM.draw(womens_singles).match('2.30').winner(women.Badosa).in_sets(2)  # (   ) Paula Badosa  OR  (   ) Anastasia Pavlyuchenkova
    TEAM.draw(womens_singles).match('2.31').winner(women.Tsurenko).in_sets(2)  # ( 28) Lesia Tsurenko  OR  (   ) Rebeka Masarova
...
def mens_singles_round_2(mens_singles):
    TEAM.draw(mens_singles).match('2.1').winner(men.Djokovic).in_sets(4)  # (   ) Alexei Popyrin  OR  (  1) Novak Djokovic
    TEAM.draw(mens_singles).match('2.2').winner(men.Etcheverry).in_sets(4)  # ( 30) Tomas Martin Etcheverry  OR  (   ) Gael Monfils
    TEAM.draw(mens_singles).match('2.3').winner(men.Mannarino).in_sets(3)  # ( 20) Adrian Mannarino  OR  (   ) Jaume Munar
    TEAM.draw(mens_singles).match('2.4').winner(men.Shelton).in_sets(4)  # ( 16) Ben Shelton  OR  (   ) Christopher O'Connell
    TEAM.draw(mens_singles).match('2.5').winner(men.Fritz).in_sets(5)  # ( 12) Taylor Fritz  OR  (   ) Hugo Gaston
    TEAM.draw(mens_singles).match('2.6').winner(men.Marozsan).in_sets(4)  # ( 22) Francisco Cerundolo  OR  (   ) Fabian Marozsan
    TEAM.draw(mens_singles).match('2.7').winner(men.Musetti).in_sets(4)  # (   ) Luca Van Assche  OR  ( 25) Lorenzo Musetti
    TEAM.draw(mens_singles).match('2.8').winner(men.Tsitsipas).in_sets(3)  # (  7) Stefanos Tsitsipas  OR  (   ) Jordan Thompson
    TEAM.draw(mens_singles).match('2.9').winner(men.Sinner).in_sets(3)  # (  4) Jannik Sinner  OR  (  Q) Jesper de Jong
    TEAM.draw(mens_singles).match('2.10').winner(men.Baez).in_sets(5)  # (   ) Daniel Elahi Galan  OR  ( 26) Sebastian Baez
    TEAM.draw(mens_singles).match('2.11').winner(men.Tiafoe).in_sets(4)  # (   ) Tomas Machac  OR  ( 17) Frances Tiafoe
    TEAM.draw(mens_singles).match('2.12').winner(men.Khachanov).in_sets(4)  # ( 15) Karen Khachanov  OR  (  Q) Aleksandar Kovacevic
    TEAM.draw(mens_singles).match('2.13').winner(men.Alex_de_Minaur).in_sets(4)  # (   ) Matteo Arnaldi  OR  ( 10) Alex de Minaur
    TEAM.draw(mens_singles).match('2.14').winner(men.Kotov).in_sets(4)  # (   ) Pavel Kotov  OR  (  Q) Flavio Cobolli
    TEAM.draw(mens_singles).match('2.15').winner(men.Halys).in_sets(4)  # (   ) Quentin Halys  OR  ( 29) Sebastian Korda
    TEAM.draw(mens_singles).match('2.16').winner(men.Rublev).in_sets(4)  # (  5) Andrey Rublev  OR  (   ) Christopher Eubanks
    TEAM.draw(mens_singles, '2.17').matchup(1, men.Cazaux, 2, men.Rune).select(2,4)  # ( WC) Arthur Cazaux  OR  (  8) Holger Rune
    TEAM.draw(mens_singles, '2.18').matchup(1, men.Fils, 2, men.Griekspoor).select(2,4)  # (   ) Arthur Fils  OR  ( 28) Tallon Griekspoor
    TEAM.draw(mens_singles).match('2.19').winner(men.Humbert).in_sets(4)  # ( 21) Ugo Humbert  OR  (   ) Zhizhen Zhang
    TEAM.draw(mens_singles).match('2.20').winner(men.Hurkacz).in_sets(3)  # (  Q) Jakub Mensik  OR  (  9) Hubert Hurkacz
    TEAM.draw(mens_singles, '2.21').matchup(1, men.Kokkinakis, 2, men.Dimitrov).select(2,4)  # (   ) Thanasi Kokkinakis  OR  ( 13) Grigor Dimitrov
    TEAM.draw(mens_singles).match('2.22').winner(men.Davidovich_Fokina).in_sets(5)  # (   ) Nuno Borges  OR  ( 23) Alejandro Davidovich Fokina
    TEAM.draw(mens_singles).match('2.23').winner(men.Auger_Aliassime).in_sets(4)  # ( 27) Felix Auger-Aliassime  OR  (  Q) Hugo Grenier
    TEAM.draw(mens_singles).match('2.24').winner(men.Medvedev).in_sets(3)  # (  3) Daniil Medvedev  OR  (   ) Emil Ruusuvuori
    ...
    TEAM.draw(mens_singles).match('2.30').winner(men.Struff).in_sets(4)  # ( 24) Jan-Lennard Struff  OR  (   ) Miomir Kecmanovic
...
def womens_singles_round_1(womens_singles):
    ...
    TEAM.draw(womens_singles, '1.3').matchup(1, women.Fiona_Ferro, 2, women.McCartney_Kessler).select(1,2)  # (  Q) Fiona Ferro  OR  ( WC) McCartney Kessler
    ...
    TEAM.draw(womens_singles, '1.6').matchup(1, women.Kayla_Day, 2, women.Tomova).select(1,2)  # (   ) Kayla Day  OR  (   ) Viktoriya Tomova
    TEAM.draw(womens_singles, '1.7').matchup(1, women.Cristian, 2, women.Siniakova).select(2,3)  # (   ) Jaqueline Cristian  OR  (   ) Katerina Siniakova
    TEAM.draw(womens_singles, '1.8').matchup(1, women.Golubic, 2, women.Kudermetova_Veronika).select(2,3)  # (   ) Viktorija Golubic  OR  ( 15) Veronika Kudermetova
    ...
    TEAM.draw(womens_singles, '1.10').matchup(1, women.Martic, 2, women.Ajla_Tomljanovic).select(2,2)  # (   ) Petra Martic  OR  (   ) Ajla Tomljanovic
    ...
    TEAM.draw(womens_singles, '1.14').matchup(1, women.Cocciaretto, 2, women.Lulu_Sun).select(1,3)  # (   ) Elisabetta Cocciaretto  OR  (  Q) Lulu Sun
    ...
    TEAM.draw(womens_singles, '1.19').matchup(1, women.Maria, 2, women.Osorio).select(1,2)  # (   ) Tatjana Maria  OR  (   ) Camila Osorio
    TEAM.draw(womens_singles, '1.20').matchup(1, women.Shnaider, 2, women.Paolini).select(2,3)  # (   ) Diana Shnaider  OR  ( 26) Jasmine Paolini
    ...
    TEAM.draw(womens_singles, '1.23').matchup(1, women.Stephens, 2, women.Gadecki).select(1,2)  # (   ) Sloane Stephens  OR  ( WC) Olivia Gadecki
    ...
    TEAM.draw(womens_singles, '1.26').matchup(1, women.Boulter, 2, women.Yuan).select(2,3)  # (   ) Katie Boulter  OR  (   ) Yue Yuan
    TEAM.draw(womens_singles, '1.27').matchup(1, women.Raducanu, 2, women.Rogers).select(2,3)  # (   ) Emma Raducanu  OR  (   ) Shelby Rogers
    TEAM.draw(womens_singles, '1.28').matchup(1, women.Yafan_Wang, 2, women.Cirstea).select(2,3)  # (   ) Yafan Wang  OR  ( 22) Sorana Cirstea
    TEAM.draw(womens_singles, '1.29').matchup(1, women.Zhu, 2, women.Oceane_Dodin).select(1,2)  # ( 29) Lin Zhu  OR  (   ) Oceane Dodin
    TEAM.draw(womens_singles, '1.30').matchup(1, women.Renata_Zarazua, 2, women.Trevisan).select(1,3)  # (  Q) Renata Zarazua  OR  (   ) Martina Trevisan
    ...
    TEAM.draw(womens_singles, '1.38').matchup(1, women.Alizé_Cornet, 2, women.Maria_Timofeeva).select(1,3)  # ( WC) Alizé Cornet  OR  (  Q) Maria Timofeeva
    ...
    TEAM.draw(womens_singles, '1.41').matchup(1, women.Garcia, 2, women.Naomi_Osaka).select(1,3)  # ( 16) Caroline Garcia  OR  (   ) Naomi Osaka
    ...
    TEAM.draw(womens_singles, '1.55').matchup(1, women.Korpatsch, 2, women.Burrage).select(2,3)  # (   ) Tamara Korpatsch  OR  (   ) Jodie Burrage
    ...
    TEAM.draw(womens_singles, '1.58').matchup(1, women.Podoroska, 2, women.Zidansek).select(1,2)  # (   ) Nadia Podoroska  OR  (   ) Tamara Zidansek
    TEAM.draw(womens_singles, '1.59').matchup(1, women.Townsend, 2, women.Badosa).select(1,3)  # (   ) Taylor Townsend  OR  (   ) Paula Badosa
    ...
    TEAM.draw(womens_singles, '1.62').matchup(1, women.Masarova, 2, women.Sasnovich).select(1,3)  # (   ) Rebeka Masarova  OR  (   ) Aliaksandra Sasnovich
    TEAM.draw(womens_singles, '1.63').matchup(1, women.Bogdan, 2, women.Fruhvirtova_Brenda).select(1,3)  # (   ) Ana Bogdan  OR  (  Q) Brenda Fruhvirtova
...
def mens_singles_round_1(mens_singles):
    ...
    TEAM.draw(mens_singles, '1.35').matchup(1, men.Fils, 2, men.Vesely).select(1,4)  # (   ) Arthur Fils  OR  (   ) Jiri Vesely
    TEAM.draw(mens_singles, '1.36').matchup(1, men.Safiullin, 2, men.Griekspoor).select(2,4)  # (   ) Roman Safiullin  OR  ( 28) Tallon Griekspoor
    TEAM.draw(mens_singles, '1.37').matchup(1, men.Humbert, 2, men.Goffin).select(1,4)  # ( 21) Ugo Humbert  OR  (  Q) David Goffin
    TEAM.draw(mens_singles, '1.38').matchup(1, men.Zhang_Zhizhen, 2, men.Coria).select(1,4)  # (   ) Zhizhen Zhang  OR  (   ) Federico Coria
    TEAM.draw(mens_singles, '1.39').matchup(1, men.Shapovalov, 2, men.Jakub_Mensik).select(1,3)  # (   ) Denis Shapovalov  OR  (  Q) Jakub Mensik
    ...
    TEAM.draw(mens_singles, '1.42').matchup(1, men.Ofner, 2, men.Kokkinakis).select(2,4)  # (   ) Sebastian Ofner  OR  (   ) Thanasi Kokkinakis
    ...
    TEAM.draw(mens_singles, '1.46').matchup(1, men.Muller, 2, men.Grenier).select(1,3)  # (   ) Alexandre Muller  OR  (  Q) Hugo Grenier
    ...
    TEAM.draw(mens_singles, '1.50').matchup(1, men.Soonwoo_Kwon, 2, men.Lukas_Klein).select(1,4)  # (   ) Soonwoo Kwon  OR  (  Q) Lukas Klein
    ...
    TEAM.draw(mens_singles, '1.56').matchup(1, men.Ramos_Vinolas, 2, men.Ruud).select(2,4)  # (   ) Albert Ramos-Vinolas  OR  ( 11) Casper Ruud
    TEAM.draw(mens_singles, '1.57').matchup(1, men.Paul, 2, men.Barrere).select(1,3)  # ( 14) Tommy Paul  OR  (   ) Gregoire Barrere
    TEAM.draw(mens_singles, '1.58').matchup(1, men.Giron, 2, men.Draper).select(2,4)  # (   ) Marcos Giron  OR  (   ) Jack Draper
    TEAM.draw(mens_singles, '1.59').matchup(1, men.Kecmanovic, 2, men.Watanuki).select(1,4)  # (   ) Miomir Kecmanovic  OR  (   ) Yosuke Watanuki
    TEAM.draw(mens_singles, '1.60').matchup(1, men.Hijikata, 2, men.Struff).select(2,4)  # (   ) Rinky Hijikata  OR  ( 24) Jan-Lennard Struff
    TEAM.draw(mens_singles, '1.61').matchup(1, men.Bublik, 2, men.Sumit_Nagal).select(1,4)  # ( 31) Alexander Bublik  OR  (  Q) Sumit Nagal
    TEAM.draw(mens_singles, '1.62').matchup(1, men.Mcdonald, 2, men.Shang).select(1,3)  # (   ) Mackenzie McDonald  OR  ( WC) Juncheng Shang
