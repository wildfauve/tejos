...
def womens_singles_round_4(womens_singles):
    ...
    TEAM.draw(womens_singles, '4.7').matchup(1, women.Andreeva_Mirra, 2, women.Krejcikova).select(1,3)  # (   ) Mirra Andreeva  OR  (  9) Barbora Krejcikova
    TEAM.draw(womens_singles, '4.8').matchup(1, women.Sabalenka, 2, women.Anisimova).select(1,3)  # (  2) Aryna Sabalenka  OR  (   ) Amanda Anisimova
...
def mens_singles_round_4(mens_singles):
    ...
    TEAM.draw(mens_singles, '4.2').matchup(1, men.Fritz, 2, men.Tsitsipas).select(2,4)  # ( 12) Taylor Fritz  OR  (  7) Stefanos Tsitsipas
    TEAM.draw(mens_singles, '4.3').matchup(1, men.Sinner, 2, men.Khachanov).select(1,5)  # (  4) Jannik Sinner  OR  ( 15) Karen Khachanov
...
def womens_singles_round_3(womens_singles):
    ...
    TEAM.draw(womens_singles).match('3.14').winner(women.Krejcikova).in_sets(2)  # (  Q) Storm Hunter  OR  (  9) Barbora Krejcikova
    TEAM.draw(womens_singles).match('3.15').winner(women.Badosa).in_sets(2)  # (   ) Amanda Anisimova  OR  (   ) Paula Badosa
...
def mens_singles_round_3(mens_singles):
    TEAM.draw(mens_singles).match('3.1').winner(men.Djokovic).in_sets(4)  # ( 30) Tomas Martin Etcheverry  OR  (  1) Novak Djokovic
    TEAM.draw(mens_singles).match('3.2').winner(men.Mannarino).in_sets(4)  # ( 20) Adrian Mannarino  OR  ( 16) Ben Shelton
    TEAM.draw(mens_singles).match('3.3').winner(men.Fritz).in_sets(4)  # ( 12) Taylor Fritz  OR  (   ) Fabian Marozsan
    TEAM.draw(mens_singles).match('3.4').winner(men.Tsitsipas).in_sets(4)  # (   ) Luca Van Assche  OR  (  7) Stefanos Tsitsipas
    ...
    TEAM.draw(mens_singles, '3.15').matchup(1, men.Kecmanovic, 2, men.Paul).select(2,5)  # (   ) Miomir Kecmanovic  OR  ( 14) Tommy Paul
...
def womens_singles_round_2(womens_singles):
    ...
    TEAM.draw(womens_singles, '2.12').matchup(1, women.Kasatkina, 2, women.Stephens).select(1,2)  # ( 14) Daria Kasatkina  OR  (   ) Sloane Stephens
    TEAM.draw(womens_singles, '2.13').matchup(1, women.Boulter, 2, women.Zheng).select(2,2)  # (   ) Katie Boulter  OR  ( 12) Qinwen Zheng
    TEAM.draw(womens_singles, '2.14').matchup(1, women.Yafan_Wang, 2, women.Raducanu).select(1,3)  # (   ) Yafan Wang  OR  (   ) Emma Raducanu
    TEAM.draw(womens_singles, '2.15').matchup(1, women.Oceane_Dodin, 2, women.Trevisan).select(1,3)  # (   ) Oceane Dodin  OR  (   ) Martina Trevisan
...
def mens_singles_round_2(mens_singles):
    ...
    TEAM.draw(mens_singles, '2.18').matchup(1, men.Fils, 2, men.Griekspoor).select(1,4)  # (   ) Arthur Fils  OR  ( 28) Tallon Griekspoor
    ...
    TEAM.draw(mens_singles, '2.21').matchup(1, men.Kokkinakis, 2, men.Dimitrov).select()  # (   ) Thanasi Kokkinakis  OR  ( 13) Grigor Dimitrov
    ...
    TEAM.draw(mens_singles, '2.26').matchup(1, men.Alex_Michelsen, 2, men.Lehecka).select(2,4)  # (   ) Alex Michelsen  OR  ( 32) Jiri Lehecka
    TEAM.draw(mens_singles, '2.27').matchup(1, men.Norrie, 2, men.Zeppieri).select(1,4)  # ( 19) Cameron Norrie  OR  (  Q) Giulio Zeppieri
    TEAM.draw(mens_singles, '2.28').matchup(1, men.Purcell, 2, men.Ruud).select(2,3)  # (   ) Max Purcell  OR  ( 11) Casper Ruud
